...
export class MovieService {
  ...
  getMovieCollection() {
    let url: string = 'http://3.19.27.205/api/movie/upload';
    return this.httpClient.get<GetMovieSearchResponse>(url);
  }

  getMovieCollectionAfterFilter(filter: string[]) {
    let filterUrl: string = `http://3.19.27.205/api/movie/filter/c=${filter[0]}/g=${filter[1]}/y=${filter[2]}/r=${filter[3]}/l=${filter[4]}/`;
